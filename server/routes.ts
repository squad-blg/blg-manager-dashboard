...
// Seed demo revenue data if not present
function seedDemoData() {
  const clients = storage.getClients();
  const now = new Date();

  for (const client of clients) {
    // Generate 13 months of revenue (current + 12 prior) for MoM
    for (let i = 0; i < 13; i++) {
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
      const period = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      const existing = storage.getRevenueSnapshots(client.id, "month");
      if (!existing.find((r) => r.period === period)) {
        const base = 3000 + Math.random() * 12000;
        const seasonalFactor = [0.7, 0.8, 1.0, 1.1, 1.3, 1.5, 1.6, 1.5, 1.2, 1.0, 0.8, 0.9][d.getMonth()];
        storage.upsertRevenueSnapshot({
          clientId: client.id,
          period,
          periodType: "month",
          revenue: Math.round(base * seasonalFactor * 100) / 100,
          orderCount: Math.round(10 + Math.random() * 40),
          fetchedAt: now.toISOString(),
        });
      }
    }

    // Year-over-year (current year + prior 2)
    for (let y = 0; y < 3; y++) {
      const year = String(now.getFullYear() - y);
      const existing = storage.getRevenueSnapshots(client.id, "year");
      if (!existing.find((r) => r.period === year)) {
        const base = 60000 + Math.random() * 80000;
        storage.upsertRevenueSnapshot({
          clientId: client.id,
          period: year,
          periodType: "year",
          revenue: Math.round(base * 100) / 100,
          orderCount: Math.round(200 + Math.random() * 400),
          fetchedAt: now.toISOString(),
        });
      }
    }
    ...
    for (let i = 0; i < 13; i++) {
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
      const period = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      const existing = storage.getAnalyticsSnapshots(client.id, "month");
      if (!existing.find((r) => r.period === period)) {
        const sessions = Math.round(800 + Math.random() * 3000);
        const leads = Math.round(sessions * (0.02 + Math.random() * 0.04));
        const adSpend = Math.round(500 + Math.random() * 2000);
        storage.upsertAnalyticsSnapshot({
          ...
          conversionRate: Math.round((leads / sessions) * 10000) / 100,
          impressions: Math.round(sessions * (8 + Math.random() * 12)),
          clicks: Math.round(sessions * (0.8 + Math.random() * 0.4)),
          adSpend,
          leads,
          costPerLead: leads > 0 ? Math.round((adSpend / leads) * 100) / 100 : 0,
